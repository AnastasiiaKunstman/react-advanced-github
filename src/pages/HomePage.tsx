...
export function HomePage() {
    ...
    return (
        <div className='flex justify-center pt-10 mx-auto h-screen w-screen'>
            {isError && <p className='text-centr text-red-600'>Something went wrong...</p>}

            <div className='relative w-[560px]'>
                <input
                    type='text'
                    className='border rounded-lg py-2 px-4 w-full h-[42px] mb-2'
                ...
                {dropdown && <ul className='list-none absolute top-[42px] left-0 right-0 max-h-[200px] overflow-y-scroll shadow-md bg-white'>
                    ...
                    {data?.map(user => (
                        <li
                            ...
                            className='py-2 px-4 hover:bg-black hover:text-white transition-colors cursor-pointer'>
                            {user.login}
                        </li>
                    ))}
                </ul>}
